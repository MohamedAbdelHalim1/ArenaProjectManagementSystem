                            ...
                            @foreach($project->tasks as $task)
                                @if (auth()->user()->role === 'admin' || $task->user_id === auth()->id())
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-sm text-red-500">{{ $task->deadline }}</td>
                    ...
                    @if (auth()->user()->role === 'admin')
                        ...
                        <div class="mb-6">
                            ...
                            <form action="{{ route('tasks.store') }}" method="POST">
                                ...
                                <div class="mb-4">
                                    <label for="title" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Task Deadline</label>
                                    <input type="date" id="deadline" name="deadline" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-800 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                    @error('deadline')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
